...
const findKingSquare = (chess: Chess, isWhite: boolean): number => {
  ...
  for (let r = 0; r < 8; r++) {
    for (let f = 0; f < 8; f++) {
      ...
      if (piece && piece.type === 'k' && piece.color === (isWhite ? 'w' : 'b')) {
...
const autoPromotionSquare = (from: number, to: number, board: Square[]): string => {
...
const GameArea: React.FC<GameAreaProps> = () => {
  ...
  const rawControl = gameData?.timeControl && gameData.timeControl > 0 ? gameData.timeControl : 180;
  ...
  const syncClockFromServer = useCallback(
    (timers: { white: number; black: number }, isWhiteMove: boolean, source: string) => {
      ...
      const prevActive = prev ? (prev.isWhiteMove ? prev.white : prev.black) : null;
      const drift =
        prevActive !== null
          ? Math.abs(prevActive - (isWhiteMove ? timers.white : timers.black))
          : Infinity;
      ...
      if (shouldHardSync) {
        ...
        console.debug(
          '[clock-anchor] soft resync | source:',
          source,
          '| drift:',
          `${drift.toFixed(2)}s`,
          '| ignored from UI'
  ...
  const clockRefTick = useRef({
    white: initialClock(gameData?.timeWite),
    black: initialClock(gameData?.timeBlack),
  });
  ...
  useEffect(() => {
    ...
    if (
      initialClock(gameData?.timeWite) === fallbackTime &&
      initialClock(gameData?.timeBlack) === fallbackTime
    ) {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      const target = Math.max(0, isWhiteMove ? anchor.white - elapsed : anchor.black - elapsed);
  ...
  useEffect(() => {
    ...
    const handleMoveMade = (message: unknown) => {
      ...
      if (msg.timers) {
        const nextIsWhite =
          msg.nextTurn === 'w' || msg.nextTurn === true || msg.nextTurn === undefined
            ? true
            : false;
        syncClockFromServer(
        ...
        if (validMoves.includes(boardIdx)) {
          ...
          try {
            ...
            if (move) {
              ...
              next[boardIdx] = {
                _id: boardIdx,
                figure: move.color === 'w' ? move.piece.toUpperCase() : move.piece,
              };
              setBoard(next);
    ...
    <div className={s.pageWrapper}>
      ...
      <div className={s.gameContainer}>
        ...
        <div className={s.boardWrap}>
          <button
            className={s.flipBtn}
            onClick={() => setShouldFlipBoard((prev) => !prev)}
            title="Flip"
            type="button"
          >
            ⇅
